...
import { getReceivedFeedbacks } from "../../../api/feedbackApi";

const WorkspaceReceivedPage = () => {
  const navigate = useNavigate();
  const roleConfig = useRoleConfig();

  const { feedbacks, isLoading, activeTab, openCount, resolvedCount, setActiveTab } =
    useWorkspaceFeedbacks({
      fetchFn: (status) => getReceivedFeedbacks(status),
    });
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <Header
        ...
        subtitle="Track and respond to feedbacks assigned to you"
      />
...
export default WorkspaceReceivedPage;