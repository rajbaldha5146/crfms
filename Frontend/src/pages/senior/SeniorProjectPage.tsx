...
const SeniorProjectPage = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      setIsLoading(true);
      const [projectsRes, hierarchyRes] = await Promise.all([
        getMyProjectCards(),
      ...
      console.error("Failed to load senior project data", err);
    } finally {
      setIsLoading(false);
    }
  }, [projectId]);
  ...
  if (!project && !isLoading) {
    return (
      <div className="min-h-screen bg-slate-50">
        <Header title="Project Not Found" subtitle="The requested project could not be loaded" />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between gap-4 mb-6">
          ...
          <Button
            variant="primary"
            onClick={() => navigate(`/senior/projects/${projectId}/create-feedback`)}
          >
            Submit Feedback
          </Button>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            <div className="mb-6">
              ...
              <p className="text-[13px] text-slate-500 mt-1">View and manage your assigned team members</p>
            </div>
                ...
                {members.map((member) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (!isOwnProfile) {
                          navigate(`/senior/projects/${projectId}/users/${member.userId}`);
                        }
                      }}
                      ...
                      <div className="flex items-center gap-4 mb-5">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <h3 className="text-[15px] font-semibold text-slate-900 truncate">{member.fullName}</h3>
                            {isOwnProfile && (
                          ...
                          {member.reportingPersonName && (
                            <div className="mt-2 flex items-center gap-1.5 text-[11px]">
                              ...
                              <span className="font-semibold text-slate-700">{member.reportingPersonName}</span>
                            </div>
                          )}
                        </div>
                      </div>
