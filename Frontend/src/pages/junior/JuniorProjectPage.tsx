...
const JuniorProjectPage = () => {
  ...
  const loadProject = async () => {
    try {
      ...
      const found = response.data.find((p) => p.id === Number(projectId));
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <Header
        title={project?.name ?? "Project Feedback"}
        subtitle="Your personal feedback inbox for this project"
      />
        ...
        {isLoading ? (
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center shadow-sm">
            ...
            <p className="text-[13px] text-slate-500 mt-2 max-w-sm mx-auto">
              You have no {status} feedbacks for this project yet.
            </p>
          </div>
        ) : (
...
export default JuniorProjectPage;
